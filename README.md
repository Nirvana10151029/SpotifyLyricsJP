# Spotify Lyrics JP v2.0.14 — Spotify内蔵版

Spotify公式サイトからインストールしたWindows版Spotifyへ、日本語訳付きの同期歌詞パネルを追加します。歌詞はSpotifyの右側に表示されるため、別ウィンドウを並べる必要はありません。

[Spotify Lyrics JP v2.0.14をダウンロード](https://github.com/Nirvana10151029/SpotifyLyricsJP/raw/refs/heads/main/SpotifyLyricsJP-Spicetify-2.0.14.zip)
...
- Spotify全体から黒背景を排除し、中央画面・標準歌詞・サイドバー・プレイヤーを桜色に合う優しい色へ統一
...
- 無料翻訳、Gemini自然訳、DeepL翻訳、Claude Haiku自然訳、GPT自然訳を画面から切り替え
- 翻訳結果を最大10,000曲までIndexedDBへ保存し、同じ歌詞の再生でAPIを呼び直さない
- 日本語だけの歌詞は翻訳APIを呼ばず、翻訳キャッシュにも保存しない
- 原文表示・自動スクロールを切り替え
...
Gemini・DeepL・Claude・GPTを使う場合は、歌詞パネルの `API設定` を押し、利用するサービスのAPIキーを入力してください。

- Gemini：[Google AI Studio](https://aistudio.google.com/app/apikey)
- DeepL：[DeepL API](https://www.deepl.com/ja/your-account/keys)
- Claude：[Claude Platform](https://platform.claude.com/settings/keys)
- GPT：[OpenAI Platform](https://platform.openai.com/api-keys)

Claude自然訳はClaude Haiku 4.5、GPT自然訳はOpenAI Responses APIとGPT-5.6 Solを使用します。Claude/ChatGPTの月額プランと各APIの料金は別です。選択したAI翻訳が失敗した場合は、その曲の残りを無料翻訳へ自動で切り替えます。

## APIキーとプライバシー

- APIキーはこのPCのSpotify用ローカルストレージ、最大10,000曲分の翻訳キャッシュは大容量のIndexedDBに保存されます。
- 以前の最大500曲キャッシュは、v2.0.12以降の初回起動時にIndexedDBへ自動移行します。
...
- セットアップが成功表示なのに「歌詞JP」が出ない場合：最新版の `START-INSTALL.bat` を実行します。必要な拡張機能を再適用します。
- Spotify側の更新で右パネルAPIが使えない場合：自動で互換パネルへ切り替わり、右下の桜色の「歌詞JP」ボタンから開閉できます。
