...
try {
    Write-Host ''
    Write-Host 'Spotify Lyrics JP v2.0.0 セットアップ' -ForegroundColor Cyan
    ...
    if (-not (Test-Path -LiteralPath $spotifyExe -PathType Leaf)) {
        ...
        Write-Host '開いたページからSpotifyをインストール後、INSTALL.batをもう一度実行してください。'
    ...
    Write-Host ''
    [void](Read-Host 'Enterを押すとこの画面を閉じます')
} catch {
    Write-Host ''
    Write-Host 'セットアップを完了できませんでした。' -ForegroundColor Red
    Write-Host $_.Exception.Message -ForegroundColor Red
    Write-Host ''
    [void](Read-Host '内容を確認してからEnterを押してください')
    exit 1
}
